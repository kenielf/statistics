...
def help_print():
    print(
        f"{FA}Invalid Options...{SR}",
        f"{FG}Usage:{SR} python3 pystatcalc.py {FG}[-h] [-t] [-g] [--help] [--terminal] [--graphical]{SR}\n",
        f"optional arguments:",
        f"  {FG}-h{SR}              shows this help message and exits",
        f"  {FG}-t{SR}              forces terminal output (GUI by default)",
        f"  {FG}-g{SR}              forces graphpical interface",
        f"  {FG}--help{SR}          same as -h",
        f"  {FG}--terminal{SR}      same as -t",
        f"  {FG}--graphical{SR}     same as -g",
        sep="\n"
    )
...
class Terminal(Interface):
    ...
    def input_window(self):
        ...
        while True:
            try:
                name = input(f"{FG}What are the values?{SR}\n")
                self.table.rol = input_validation(name)
                break
            except ValueError:
                print(f"{FA}List is Invalid.{SR}\nPlease input a valid list.")
                continue
        print(
            f"\n{FG}Your formatted input is:\n{SR}"
            f"{', '.join(str(v) for v in self.table.rol)}"
        )

    def central_tendency_window(self):
        ctw_header = str('┌' + '─' * 126 + '┐')
        ctw_header_c = str('│' + FG + " {:<124} ".format("Central Tendency Table") + SR + '│')
    ...
    def gen_information_window(self):
        ...
        giw_header_c = str('│' + FG
                           + " {:<60} ".format(f"ROL [{self.table.class_value}l] - Interval: [{self.table.interval}]")
                           + SR + '│' + FG
    ...
    def reg_table_window(self):
        ...
        rt_header_c = str('│'
                          ...
                          + FG + "{:<7}".format(" Fac% ") + SR + '│'
                          + FG + "{:<8}".format(" FᵢXᵢ ") + SR + '│'
                          + FG + "{:<12}".format(" |Xᵢ- x̅| ") + SR + '│'
                          + FG + "{:<13}".format(" (Xᵢ- x̅)² ") + SR + '│'
                          + FG + "{:<14}".format(" Fᵢ|Xᵢ- x̅| ") + SR + '│'
                          + FG + "{:<14}".format(" Fᵢ(Xᵢ- x̅)² ") + SR + '│')
        rt_separator = str('├'
                           + '─' * 17 + '┼'
                           + '─' * 6 + '┼'
